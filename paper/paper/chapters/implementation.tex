...
\label{sec:implementation}
	...
	\emph{Model Adaptation} to address \remove{variation point} VP1. Afterwards, we discuss how \emph{Model
	Instance Adaptation} \change{enables variation point}{supports} VP2.
...
\subsection{Model Adaptation (VP1)}
	...
	the basic	concepts such as \texttt{Type\remove{s}}, \texttt{Propert\change{ies}{y}}, \texttt{Operation\remove{s}} and \texttt{Parameter\remove{s}}
	...
	e.g., the OCL\remove{2} parser	can determine the \texttt{Type} of OCL expressions.
...
% 	solve similar problems when working with multiple model instantiations. 
	...
	\note{Discuss the following sentence. Right or wrong? What about type matching? Claas: I argue that this is correct.
	When reflecting on ModelInstanceElements, the result is a Type defined at the layer above. When querying on a ModelElement
	the result is a Type defined at the same layer.}
  The most important difference between \add{the} Model Types and Model Instance Types
	...
	\change{Like the model adapter component, t}{T}he model instance adapter component also 
