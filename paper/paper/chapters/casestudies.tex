...
In this section we present three case studies to demonstrate the \change{power}{benefits} of the presented
\change{generic adaptation architecture}{variability}.
...
these SEPA messages. We implemented an XSD model and an XML model instance adaptation 
...
About 120 constrains that are provided by the demo were used to test our adaptation mechanism. 
...
\note{Michael: again: model instance, but speak of XML meta-model} For the XML instances, an \textit{XML Instance Adapter} component was implemented,
...
\begin{figure}[!t]
	...
	\caption{\note{Michael: I know that this is not correct, but we should replace essentialOcl with model types for consistency} In the standard library case study, VP1 is bound to the \textit{EMF Ecore Model Adapter}. 
