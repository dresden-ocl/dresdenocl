...
generic adaptation architecture. \add{The case studies use different kinds of models
...
as defined in \cite{warmer:ocl}. \change{The case study originally}{This example} was designed by \textsc{Warmer
...
into DresdenOCL, \change{an}{the} \textit{XSD Model Adapter} component \remove{was implemented that} 
...
\change{For the XML
documents, an}{The} \textit{XML Instance Adapter} component \remove{was implemented that}
...
in order to check for inconsistencies between \change{both}{these} instances. In this example we 
...
This implementation is realised in Java. As there is no code generator for the Model Types, \change{this}{the manual implementation} can lead to inconsistencies between 
