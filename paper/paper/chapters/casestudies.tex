...
check\add{ed} \change{WFRs}{\textit{Well-Formedness Rules (WFRs)}} for the OCL standard library of DresdenOCL. 
DresdenOCL's standard library is explicitly modelled as 
an instance of the \change{PivotModel}{model types}, describing predefined OCL types like \texttt{Integer}, 
...
current OCL \change{standard}{specification} \add{or not}.
...
This implementation is provided through Java code. As the \change{PivotModel}{\textit{Model Types Editor} of Dresden OCL}
...
In order to evaluate the WFRs, the \change{PivotModel}{model types} need\remove{s} to be loaded as a model (cf. Fig. 
\ref{fig:casestudy03}) \note{Claas: Model Figure!} via the \textit{Ecore Model Adapter}. Now, the modelled standard library 
can be loaded as an instance of the \change{PivotModel}{model types} via the \textit{XMI Model Instance Adapter}. Thus, 
we are able to evaluate the WFRs for the standard library model and can prove the correct 
structure of it according to the current OCL \change{standard}{specification}. 
