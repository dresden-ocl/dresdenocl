...
\label{sec:lessons}
  ...
  In this section we highlight some \add{challenges we faced} during the design
  and implementation of our generic adaptation architecture for DresdenOCL.
  We present \remove{our} solutions to these challenges and possible improvements.
	...
	matched to the UML class \texttt{LoyaltyAccount} in the royal and loyal \change{class diagram}{case study}.
	This matching algorithm can be rather complex and often information \remove{is hidden inside the adapters} that could be 
	used to improve the matching \add{is hidden inside the adapters}. E.g., when adapting an instance of an EMF 
	...
	types using an \change{implementation}{model instance} specific matcher and end by trying to 
	simply match the type names as currently done.
	
	\paragraph{Element Unwrapping\add{:}}
	...
	\change{on the \texttt{ModelInstanceElements}}{inside the adapters}. E.g., to invoke an operation of a\add{n adapted} Java 
	\change{implementation}{object} we require \texttt{java.lang.Objects} as parameters instead of 
	...
	Fortunately, model instances providing only structural information \add{(for example XML files)} do not
	...
	retrieve \texttt{Types}, \texttt{Operations}, \texttt{Properties} for the variation point VP1 are implemented \change{appropriately.
	The reflection mechanism provided by variation point VP2 is tested as well.}{or if the reflection mechanism
