...
public class UmlModelProvider extends AbstractModelProvider implements
  ...
  private static final Logger logger = Logger.getLogger(UmlModelProvider.class);
	...
	private void init() throws ModelAccessException {
	  ...
	  if (metamodel == null) {
			try {
			  ...
			  if(metamodel == null){
			      metamodel = modelmanager.loadMetaModel("file:"+resPath+"metamodels/UML15_plus_OCLMetamodel.xml", "UML_MM");
			  }
			} catch (Exception e) {
				throw new ModelAccessException("Metamodel UML not found in repository " + repository, e);
			}
	  }
	}
}
