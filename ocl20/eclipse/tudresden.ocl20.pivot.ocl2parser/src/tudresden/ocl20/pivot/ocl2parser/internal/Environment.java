...
public class Environment {
	 ...
	 * <i>params</i>. The search is being performed with including
	 * the parent environments. If in no environment of this chain
	 * the operation is found null will be returned, otherwise the operation. 
	 * @param name the name of the operation to be looking for
	 ...
	 * that the last element of the path name denotes a type. If
	 * the type is not found or the path name is ambiguous then
	 * the return value is null. Otherwise the type will be returned.
	 * @param pathName the path name to be look up for
	 * @return the type that was found or null if no type is found or the
	 * path is ambiguous
	 */
	 ...
	 * that the last element of the path name is the property and the element before
	 * last is a type. If no property is found, null will be returned, also when the
	 * property is ambiguous. Otherwise the property will  be returned.
	 * @param pathName the path name to be look up for
	 * @return the property or null if no property with this path name exists or the
	...
	public Property lookupProperty(List<String> pathName) {
	    ...
	    if (contextualType == null) return null;
	...
	/**
	 * Looks for an operation with the given path name. The assumption is that
	 * the last element of the path name denotes an operation name. And the element
	 ...
	 * is not only search by their name but also by the parameters. If an operation
	 * with the given parameters is found, this operation will be returned. If the operation
	 * path name is ambiguous or no operation exists then null will be returned.
	 * @param pathName the path name to be looking for
	...
	public Operation lookupOperation(List<String> pathName, List<Type> parameterTypes) {
	    ...
	    // find the contextual type
	    Type contextualType = findContextualType(pathName);

	    // the operation name is the last element of the path name
	    String operationName = pathName.get(pathName.size()-1);
	 ...
	 * that the path name last element is an operation or attribute name. So
	 * the element before last must be a type. This method looks for this
	 * type. If it is found, it will be returned. If the path name for this
	 * type is ambiguous, null will be returned. If the type is not found,
	 * null will also be returned.
	 * @param pathName the path name to be looking for the type
	 * @return the type or null if the type doesn't exists or the path name
	 * for that type is ambiguous
	 */
	...
	/**
	 * Creates a nested environment. That means a new environment is created with <i>this</i> as
	 * parent. The model is the same, but the new environment has no other elements.
	 * @return
	 */
	...
	public Variable lookupImplicitProperty(String name) {
		...
		if (parent != null) return parent.lookupImplicitProperty(name);
	...
	public Operation lookupImplicitOperation(String name, List<OclExpression> params) {
		 ...
		 * Because the lookup operation  if type expect a list of types.
		 */
		...
		/*
		 * We must lookup in the contextual classifier of the environment for
		 * the operation. This is used by the iterator expression.
		 */
		/*if (this.contextualClassifier != null) {
			Operation foundClassifierOperation = null;
			foundClassifierOperation = contextualClassifier.lookupOperation(name, typeList);
			if (foundClassifierOperation != null) return foundClassifierOperation; 
		}*/
	...
	/**
	 * The given <i>operation</i> will be query for its owning type. If no owning type exists,
	 * it is a little bit strange, but the method will returned null. If an owning type
	 * exists, this will be packed into a variable. This variable has no init expression
	 * and the name beomes "".
	 * @param operation the operation to be query for the owning type
	 ...
	 * is found, it will be returned, otherwise null. 
	 * @param pathName the path name to be looked up for
	 ...
	 * @param env the parent to be set
	 */
	 ...
	 * variable will not added to the environment and the return value will be false.
	 * If the variable doesn't already exists in the environment, it will added
	 * to the environment and the return value will be true.
	...
	public OclLibrary getOclLibrary() {
		return model.getOclLibraryProvider().getOclLibrary();
	}
	
	/**
	 * Adds an implicit variable to the list of the implicit variables.
	 * An implicit variable is build in the computation of the iterator/iterate
	 * expression. This variables are used to find an implicit operation or property.
	 * In the OCL-specification there is method <i>addElement</i> with the parameter <i>Imp</i>
	 * that denotes whether the element should be searched for an implicit operation or
	 * attribute. In this environment there are two methods: this method <i>addImplicitVariable</i>
	 * and <i>addVariable</i>. The first one is for the implicit variables and the second for
	 * the variables that are defined in the let expressions. Note that in the implicit search methods
	 * the normal variables will not be searched, but the implicit variables.
	 * If the variable <i>var</i> already exists in the list, nothing happens.
	...
	/**
	 * Marks the environment as special ocl operation. This is set if one the operations
	 * 'oclIsKindOf', 'oclIsTypeOf' or 'oclAsType' is recognized. If so, we must notice this
	 * to compute a variable expression that holds the type.
