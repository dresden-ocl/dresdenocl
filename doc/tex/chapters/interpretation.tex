...
First, the constraint file defines three simple invariants that ensure that the
\model{age} of every \model{Person} must always be zero or greater than zero.
...
\eclipse{Model Browser}. We use the model import wizard of Dresden OCL to
...
and methods or to initialize attributes and methods. Such \model{def},
\model{init} and \model{body} constraints cannot be interpreted to \model{true}
or \model{false}, because their result type has not to be \model{Boolean}.
Furthermore, they can be used to alter the results of other constraints that
shall be interpreted. The \model{allConstraints.ocl} file contains a definition
constraint, which defines the method \model{getAge()} for the class
\model{Person}. Now, we want to interpret this definition constraint. We select
the constraint in the \eclipse{Model Browser} and click the \eclipse{Interpret}
...
\model{Person} instance. Therefore, OCL provides the special variable
\model{result} in postconditions which contains the result of the constrained
method's execution. Using the \eclipse{OCL Interpreter View} we cannot execute
the method \model{getAge()} and store the result in the \model{result}
...
variable type \model{Integer} and enter the value \model{25}. Then, we click
the \eclipse{OK} button (cf. Fig.~\ref{pic:interpret:interpret09}). The
...
postcondition is invoked. Listing~\ref{lst:interpret:postcondition} shows such
a postcondition. The postcondition is defined on an operation 
...
comparing to values using the \code{=} operation results in \code{false}, you
can investigate the left- and right-hand arguments of the \code{=} expression
resulting in \code{false}. To receive interim results during interpretation of
OCL expressions, open the Dresden OCL Tracer from the menu option
\eclipse{Dresden OCL > Open Dresden OCL Tracer} before you start the
interpretation. Once opened, the execution path of an OCL expression will be
logged and shown in the OCL \eclipse{Tracer View} (cf.
Figure~\ref{pic:interpret:tracer01}).

When debugging OCL expressions users typically intend to find failures. Thus,
the OCL \eclipse{Tracer View} also provides a menu to filter for special
criteria, e.g. all \model{true} or \model{false} results (cf.
