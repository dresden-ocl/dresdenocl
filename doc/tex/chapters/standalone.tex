...
example at \code{https://github.com/dresden-ocl/standalone.git}. It can be used
...
The \reference{lib} folder contains all libraries that are needed when
...
the application and others. The \reference{plugins} folder contains all jar
...
\code{Stand\-a\-lone\-Fa\-cade}. The method takes two arguments, the first
...
\code{I\-In\-ter\-pre\-ta\-tion\-Re\-sult}s that contain the context
(\code{getModelObject()}), the executed constraint (\code{get\-Con\-straint()})
and the result of the interpretation (\code{getResult()}).
...
IOcl2JavaSettings settings))}. In this case the method returns a list of
...
\code{https://github.com/dresden-ocl/standalone.git}.
