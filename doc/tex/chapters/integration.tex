...
Since the release 2.2.0, Dresden OCL contains an \keyword{Integration Facade},
...
\code{org.dresdenocl.facade}. Please be aware of the fact that if you
...
Each metamodel has its own \code{IModelProvider} that can be accessed by using 
...
\code{org.dresdenocl.modelinstancetype.java})).
...
the \keyword{Model Instance Browser} of Dresden OCL's GUI.



\section{How to access the OCL Parser}
The \acs{OCL}2 Parser of \acl{DOT4Eclipse} is located in the plug-in 
\code{org.dresdenocl.\linebreak[0]language.ocl.staticsemantics} within
the package
\code{org.dresdenocl.language.ocl\linebreak[0].\linebreak[0]re\-source\linebreak[0].ocl}.
The parser provides a very simple interface and can be used as shown in Listing
\ref{lst:integration:parserConstraints}. You have to provide an \code{IModel}
and an \code{URI} where the \acs{OCL} file can be found. You can specify whether
constraints and definitions should be added to the \code{IModel} by a third
parameter named \code{addToModel}. It can be left out and is then assumed to be
\code{true}.
