...
To install \textit{Dresden OCL2 for Eclipse} as \textit{Eclipse} plug-in, you need to have the jar archives of the toolkit. The jar archives are available at \reference{http://dresden-ocl.\linebreak[0]sourceforge.net/}. You need to copy the jar archives into the ``plugins'' directory of your \textit{Eclipse SDK} distribution. Then you can start the \textit{Eclipse SDK} and you can work with \textit{Dresden OCL2 for Eclipse}.
...
On the one hand you can import the plug-ins from your file system, if you have already downloaded them (e.g. from \reference{http://sourceforge.net/\linebreak[0]projects/\linebreak[0]dresden-ocl/} as a source code distribution). One the other hand you can import the plug-ins directly from the \textit{SVN} (\textit{Subversion}) of \textit{Dresden OCL2 for Eclipse}. Both possibilities are described below.
...
After installing \textit{Subclipse}, a new \textit{Eclispe perspective} for access to \textit{SVN} should exist. The perspective can be opened via the menu ``Window > Open Perpective > Other... > SVN Repository Exploring''. In the view \textit{SVN Repository} you can add a new repository (see figure \ref{pic:SVN}) using the URL \reference{https://dresden-ocl.\linebreak[0]svn.sourceforge.net/svnroot/dresden-ocl/}. After pressing the button\linebreak[0] ``Finish'' the SVN repository root should the visible in the \textit{repository view}.
	
To checkout the plug-ins, you now select them in the repository directory \reference{trunk/ocl20forEclipse/eclipse} and use the ``Checkout...'' function in the context menu (see figure \ref{pic:Checkout}). The given settings could be used and after a click on the ''Finish'' button the plug-ins should be imported.
...
After executing the build script successfully you need to update the projects in your workspace. Update the project \reference{tudresden.ocl20.pivot.oclparser} via context menu (``Refresh'', see figure \ref{pic:Refresh}).
...
This tutorial explains \textit{Dresden OCL2 for Eclipse} using the \textit{Simple example} which is located in the plug-in package \reference{tudresden.ocl20.pivot.examples.\linebreak[0]simple}. Figure \ref{pic:Simple} shows the class diagram which is described by the \textit{Simple example}.
...
\begin{tabular}[h]{|p{5.2cm}|p{6cm}|}
  ...
  Plug-in package & \reference{tudresden.ocl20.pivot.examples.\linebreak[0]simple}\\
  Meta model & UML 1.5 or UML 2.0\\
  Model & \reference{model/simple.xmi} (with UML 1.5)\newline \reference{model/simple.uml} (with UML 2.0)\\
  OCL expressions & \reference{constraints/invariants.ocl}\\
  Model instance & \reference{src/tudresden.ocl20.pivot.\linebreak[0]examples.simple/\linebreak[0]ModelProviderClass.java}\\
  \hline
  \hline
  \textbf{Royal and Loyal example} & \\
  Plug-in package & \reference{tudresden.ocl20.pivot.examples.\linebreak[0]royalandloyal}\\
  Meta model & UML 1.5 or UML 2.0\\
  Model & \reference{model/royalsandloyals.xmi}\newline (with UML 1.5) \newline \reference{model/royalsandloyals.uml}\newline (with UML 2.0)\\
  OCL expressions & \reference{expressions/*.ocl}\\
  Model instance & \reference{src/tudresden.ocl20.pivot.\linebreak[0]examples.royalsandloyals/\linebreak[0]ModelProviderClass.java}\\
  \hline
  \hline
  \textbf{Living example} & \\
  Plug-in package & \reference{tudresden.ocl20.pivot.examples.\linebreak[0]living}\\
  Meta model & UML 1.5\\
  Model & \reference{model/UmlExample.xmi}\\
  OCL expressions & \reference{expressions/living.ocl}\\
  Model instance & \reference{src/tudresden.ocl20.pivot.\linebreak[0]examples.living/\linebreak[0]ModelProviderClass.java}\\
  \hline
  \hline
  \textbf{PML example} & \\
  Plug-in package & \reference{tudresden.ocl20.pivot.examples.\linebreak[0]pml}\\
  Meta model & Ecore\\
  Model & \reference{model/pml.ecore}\\
  OCL expressions & \reference{expressions/testpml.ocl}\\
  Model instance & \reference{model instance/Testmodell.pml}\\
  \hline
\end{tabular}
...
If the plug-ins of \textit{Dresden OCL2 for Eclipse} were installed as jar archives, the \textit{Simple example} plug-in has to be imported into the \textit{Workspace} first. Create a new Java project into your \textit{Workspace} and select the \textit{import wizard} ``General -> Archive File''; in the following window select the ``plugins'' directory into your \textit{Eclipse} root folder, select the archive \reference{tudresden.ocl20.pivot.examples.\linebreak[0]simple\_1.0.0.jar} and click the ``Finish'' button.

Now you can load a model by using the \textit{import wizard} (File~>~Import...). Select the wizard ``Dresden OCL2 Toolkit > Domain-Specific~Model''. In a new opened window you have to select a model file and a meta model for the model (see figure \ref{pic:LoadModel}). Click the button ``Browse Workspace...'' and select the file \reference{model/simple.uml} inside the \textit{Simple example project}. Then select the meta model \textit{UML2} and press the button ``Finish''.
...
After loading the model, you can load a \textit{model instance} using another \textit{import wizard}. Use the wizard ``Dresden OCL2 Toolkit > Model Instance''. You have to select a model instance (in this tutorial we used the file \reference{bin/tudresden/ocl20/\linebreak[0]pivot/examples/ModelProviderClass.class} of the \textit{simple example} and the domain specific model loaded before (see figure \ref{pic:LoadInstance}).
