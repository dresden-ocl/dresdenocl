...
Next, we have to import a second project called \model{tudresden.ocl20.pivot.\linebreak[0]examples.simple.constraints}. We can use the same mechanism explained above, but instead of a Java project we now create an AspectJ project before we import the archive file (if the wizard to create an AspectJ project is not available you have to install the \textit{AspectJ Development Tools} first). \textbf{Eventually you have also to add the JUnit4 library to the project classpath to run the JUnit test case mentioned later on.}
