...
\code{EObject}s and \acs{XML} files. This is possible because DresdenOCL
...
the interpreter has to store the value of the property \code{age}. The value has
to be copied, because if \code{age} is incremented during the method's
...
\code{tudresden.ocl20.pivot.modelinstance} can be reused.
...
\code{tudresden.ocl20.\linebreak[0]pivot.modelinstance} can be reused. During 
...
\code{tudresden.ocl20.\linebreak[0]pivot.modelinstance} exists.
...
(\code{tudresden.ocl20.pivot.modelinstancetype.java}), \acs{EMF} Ecore
(\code{tudresden.ocl20.pivot.modelinstancetype.ecore}) and \acs{XML}
(\code{tudresden.ocl20\linebreak[0].pi\-vot\linebreak[0].modelinstancetype.xml}).
