...
\emph{DresdenOCL} is developed as a project at the Technische Universität Dresden (TUD), Software Technology Group since 1999. Its latest version is released as a set of Eclipse plug-ins and thus, called \emph{\acl{DOT4Eclipse}}. DresdenOCL contains a set of OCL tools, including an OCL parser, an OCL interpreter and an OCL-to-Java code generator.
...
\begin{itemize}
	...
	\item Messages as defined in the OCL 2.2 specification are not supported by DresdenOCL.
\end{itemize}
	
	
\subsection{Different Semantics of OCL Expressions in DresdenOCL}

The current OCL 2.2 specification \cite{spec:OCL2-2} contains some inconsistencies and misses some definitions especially when evaluating \code{invalid} or \code{null} values. Thus, we had to assume or change the semantics of OCL during interpretation of some OCL statements. We tried to document the OCL semantics used in DresdenOCL where it differs from the official OCL specification \cite{spec:OCL2-2}.
...
\begin{table}
	\centering
		\begin{tabular}{|p{1.2cm}p{1.2cm}||p{1.2cm}|p{1.2cm}|p{1.2cm}|p{2.0cm}|}
			\hline
      \textbf{a}         & \textbf{b}         & \textbf{not a}   & \textbf{a or b}  & \textbf{a and b} & \textbf{a implies b} \\
			\hline
...
\begin{figure}[t]
\lstset{language=OCL, label={lst:exampleIterators}, caption={Some example Iterator Expressions and their evaluation results.}, captionpos=b}
\begin{lstlisting}
...
In DresdenOCL, iterators will result in a value as long as they can be computed, even if their collection contains \code{null} values. Listing \ref{lst:exampleIterators} shows some examples for iterator expressions and their results in DresdenOCL. E.g., an \code{any} iterator will result in \code{true} as long as one element of its source's collection fulfils its condition even if other elements are \code{null} values. On the other hand, a \code{select} iterator will result in \code{invalid} if any element in its collection is \code{null} because it cannot be determined if the \code{null} value has to belong to the result set or not.


\subsubsection{Handling of Null values}

\begin{figure}[t]
\lstset{language=OCL, label={lst:nullValues}, caption={Evaluation of null values in DresdenOCL.}, captionpos=b}
\begin{lstlisting}
null + 2 => invalid
null.asSet() => Set { }
...
\section{Supported Meta-Models and Models In DresdenOCL}
...
\begin{figure}[t]
	...
	\caption{Transitive Import of Java Classes as a Model onto DresdenOCL.}
...
\begin{enumerate}
	\item \code{*.class} files can be imported as a model. All directly referenced classes (either as types of properties and operations or their arguments) are imported as well. \textbf{Please be aware, that only byte code classes (\code{*.class}) and not source code classes \code{*.java} can be imported into Dresden OCL!}
...
\section{Supported Model Instance Types and Model Intances In DresdenOCL}
...
Besides the creation of meta-models or \acs{DSL}, the \acf{EMF} allows the generation of simple model editors that can be used to model instances of the Ecore-based models created before. E.g., you can create a simple \acs{DSL} using \acs{EMF}. Afterwards you can model an instance of this \acs{DSL} using an Ecore-generated model editor. Now, you can import your \acs{DSL} as a model into DresdenOCL and you can parse \acs{OCL} constraints defined on your \acs{DSL}. To check these constraints on the created model instance, you have to import this instance into DresdenOCL as well.
...
Besides complete models it can be useful to import single model instance elements into DresdenOCL when using DresdenOCL directly from another software application via its \acs{API}. How this is possible is documented in Subsection~\ref{sect:modelInstanceTypeAdaptation:addIMIElement}. For Ecore model instances, this mechanism can be used to add single \code{EObjects} to a model instance in DresdenOCL.
...
\begin{enumerate}
	...
	\item Similar to Ecore model instances, it is possible to add single \code{java.lang.Objects} to an existing model instances via DresdenOCL's \acs{API} at runtime as documented in Subsection~\ref{sect:modelInstanceTypeAdaptation:addIMIElement}.
\end{enumerate}
...
This chapter documented which version of OCL DresdenOCL supports. Furthermore, supported types of Meta-models, models, and model instances were presented. The following chapter will explain how to install and use DresdenOCL.