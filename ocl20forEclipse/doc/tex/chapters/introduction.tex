...
\begin{tabular}{|p{7cm}p{7cm}|}
  ...
  \hline
  UML-based model,\newline e.g., \texttt{model01.uml} &
  Java-based instance (runtime objects),\newline e.g., \texttt{Instance01.class} \\
  \hline
  Java classes,\newline e.g., \texttt{Model01.class} &
  Java-based instance (runtime objects),\newline e.g., \texttt{Instance01.class} \\
  \hline
...
(e.g., \texttt{model01.mydsl}) to interprete the specified constraints on them.
...
you have to import using the \acs{EMF}/Ecore model importer and not a \acs{UML}
model importer (since the \acs{UML} metamodel was modelled in \acs{EMF})!
...
the Java model instance importer (e.g., \texttt{Instance01.class}). Details how
...
\acs{OCL} constraint specification (e.g., \texttt{Model01.class}). A Java class
...
(e.g., \texttt{Instance01.class}).

\subsubsection{BRs for XML Schemata}
Dresden \acs{OCL} supports the definition of \acs{OCL} rules on \acl{XSDs}
