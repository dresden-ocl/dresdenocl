...
public class OclResource extends
	...
	public void reload(java.io.InputStream inputStream,
		...
		synchronized (loadingLock) {
			...
			try {
				...
				// do nothing - the resource is left unchanged if this exception is
				// thrown
			}
			resolveAfterParsing();
			isLoaded = true;
		...
		if (mapping instanceof tudresden.ocl20.pivot.language.ocl.resource.ocl.IOclURIMapping<?>) {
			...
			if (uri != null) {
				...
				try {
					...
					// we can catch exceptions here, because EMF will try to resolve again
					// and handle
					// the exception
				}
	...
	protected void runValidators(org.eclipse.emf.ecore.EObject root) {
		...
		// checking constraints provided by EMF validator classes was disabled by
		// option
		// 'disableEValidators'.
