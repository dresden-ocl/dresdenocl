...
public privileged aspect InvAspect18 {
     ...
     *       inv: let correctDate:Boolean=self[].validFrom[].isBefore( Date::now()).and( self[].validThru[].isAfter( Date::now())) in if valid[] then correctDate[].=( false) else correctDate[].=( true)</code></p>
     */
