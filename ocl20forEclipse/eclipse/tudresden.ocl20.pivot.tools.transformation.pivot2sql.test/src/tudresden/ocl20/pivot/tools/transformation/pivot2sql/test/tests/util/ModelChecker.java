...
public class ModelChecker {
			...
			if (me instanceof Table) {
				if (tables.contains(me.getName())) {
					...
					for (ModelElement m : table.getOwnedElement()) {
						if (m instanceof PrimaryKey) {
							assertNotNull("extra primary key " + m.getName() + " in table "
									+ table.getName(), table2PrimaryKey.get(table.getName()));
							assertTrue("extra primary key " + m.getName() + " in table "
									+ table.getName(), table2PrimaryKey.get(table.getName())
									.equals(m.getName()));
						}
						else if (m instanceof ForeignKey) {
							List<String> foreignKeys = table2ForeignKey.get(table.getName());
							assertNotNull("extra foreign key " + m.getName() + " in table "
									+ table.getName(), foreignKeys);
							boolean check = false;
							...
							assertTrue("extra foreign key " + m.getName() + " in table "
									+ table.getName(), check);
						}
						else if (m instanceof Column) {
							List<Tuple<String, String>> tuples =
									table2properties.get(table.getName());
							boolean check = false;
							for (Tuple<String, String> t : tuples) {
								if (t.getElem1().equals(m.getName())) {
									assertTrue("Not the same datatype of " + m.getName()
											+ " in table " + table.getName(),
											t.getElem2().equals(((Column) m).getType().getName()));
									check = true;
									break;
								}
							}
							assertTrue("the column " + m.getName() + " isn't a part of table"
									+ table.getName(), check);
						}
						else {
							fail("the element " + m.getName() + " isn't a part of table"
									+ table.getName());
						}
					}
				}
				...
				if (views.contains(me.getName())) {
					if (view.getQueryExpression() != null) {
						assertNotNull("extra query expression in table " + view.getName(),
								view2queryexpression.get(view.getName()));
		...
		for (String table : tables) {
			...
			if (table2PrimaryKey.containsKey(table)) {
				...
				if (mes.size() == 1) {
					...
					assertTrue("foreign key and primary key not same name", me1.getName()
							.equals(me2.getName()));
					if (me2 instanceof ForeignKey) {
						assertTrue("No primary key " + table2PrimaryKey.get(table)
								+ " in table " + table + " found", (me1 instanceof PrimaryKey));
					}
					else {
						assertTrue("No primary key " + table2PrimaryKey.get(table)
								+ " in table " + table + " found", (me2 instanceof PrimaryKey));
					}
				}
				else {
					fail("No distinct primary key " + table2PrimaryKey.get(table)
							+ " in table " + table + " found");
				}
				...
				for (String foreignKey : table2ForeignKey.get(table)) {
					...
					if (mes.size() == 1) {
						assertTrue("No foreign key " + foreignKey + " in table " + table
								+ " found", (mes.get(0) instanceof ForeignKey));
					}
					else if (mes.size() == 2) {
						ModelElement me1 = mes.get(0);
						ModelElement me2 = mes.get(1);
						...
						if (me2 instanceof PrimaryKey) {
							...
							assertTrue("No foreign key " + foreignKey + " in table " + table
									+ " found", (me2 instanceof ForeignKey));
						}
					}
					else {
						fail("No distinct foreign key " + foreignKey + " in table " + table
								+ " found");
					}
				}
			}
	...
	public static void sameDdl(String expected, String actual) {
		...
		for (String s : actualList) {
			assertTrue("not expected string " + s + "not in actual",
					expectedList.contains(s));
		}
	}
	...
	public static void checkMappedModel(IMappedModel mm, List<String> classes,
			Map<String, List<String>> class2attributes,
			Map<String, List<String>> class2assEnds) {

		// The mapped Model should not contain the mapped Class 'falsePositive'
		assertFalse("mapped model has class falsePositive",
				mm.isClass("falsePositive"));
		...
		for (String classname : classes) {
			...
			assertTrue("the class with the name " + classname + " isn't represent",
					mm.isClass(classname));
			...
			assertEquals("the name of mapped class is not equal (" + classname + ","
					+ mc.getName() + ")", mc.getName(), classname);

			// The class should not have the atrribute 'falsePositive'
			List<String> attributes = class2attributes.get(classname);
			assertFalse("mapped class " + classname + " has attribute falsePositive",
					mc.isAttribute("falsePositve"));
			...
			// Check if the mapped Class has all attributes specified in the
			// HashMap
			if (attributes != null) {
				for (String attribute : attributes) {
					assertTrue("the class " + classname + " has no attribute "
							+ attribute, mc.isAttribute(attribute));
					// Check if each Attribute has its Attribute Guide
					Guide attG = mc.getAttributeGuide(attribute);
					assertNotNull("the attribute guide of " + attribute + " of class "
							+ classname + " is missed", attG);
				}
			}

			// The mapped Class should not have the AssociationEnd
			// 'falsePositive'
			assertFalse("the mapped class " + classname
					+ " has an association end falsePositive",
					mc.isAssociationEnd("falsePositive"));
			try {
			...
			// Check if the mapped Class has all AssociationEnds specified in
			// the HashMap
			List<String> assEnds = class2assEnds.get(classname);
			if (assEnds != null) {
				for (String assEnd : assEnds) {
					assertTrue("the mapped class " + classname
							+ " has no association end " + assEnd,
							mc.isAssociationEnd(assEnd));
					// Check if each AssociationEnd has its AssociationEndGuide
					Guide assEG = mc.getAssociationEndGuide(assEnd);
					assertNotNull("Miss the guide of the association end " + assEnd
							+ " of class " + classname, assEG);
				}
			}
		}
	}
