...
public class TestPerformer {
	...
	public static IModel addUMLModel(File file) throws IllegalArgumentException, ModelAccessException {
		return ModelBusPlugin.getMetamodelRegistry().getMetamodel(UML2MetamodelPlugin.ID).getModelProvider().getModel(file);
	}
	...
	public static ITemplateGroup getTemplateGroup() {
		return TemplatePlugin.getTemplateGroupRegistry().getTemplateGroup(TEMPLATE_NAME);
	}
	
	public static IOcl2DeclSettings getSettings() {
		IOcl2DeclSettings oclSettings = Ocl2DeclCodeFactory.getInstance().createOcl2DeclCodeSettings();
