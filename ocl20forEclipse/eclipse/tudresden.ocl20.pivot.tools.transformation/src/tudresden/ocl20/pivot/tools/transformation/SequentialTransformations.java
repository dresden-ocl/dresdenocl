 ...
 * @param <OUT>
 */
public abstract class SequentialTransformations<IN extends EObject, BET, OUT>
		extends M2XTransformation<IN, OUT> {

	private ITransformation<IN, BET> trans1;
	private ITransformation<BET, OUT> trans2;

	@SuppressWarnings("unchecked")
	public SequentialTransformations(String modelInName, String outName,
			String tid1, String tid2) throws ModelAccessException {

		super(modelInName, outName);

		trans1 =
				(ITransformation<IN, BET>) TransformationPlugin
						.getTransformationRegistry().getTransformation(tid1, modelInName,
								"intermediateResult");
		trans2 =
				(ITransformation<BET, OUT>) TransformationPlugin
						.getTransformationRegistry().getTransformation(tid2,
								"intermediateResult", outName);
