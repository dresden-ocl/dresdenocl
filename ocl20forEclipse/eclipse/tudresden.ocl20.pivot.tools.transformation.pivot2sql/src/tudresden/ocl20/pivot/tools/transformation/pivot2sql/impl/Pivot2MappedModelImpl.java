 ...
 *
 */
public class Pivot2MappedModelImpl extends M2XTransformation<Namespace, IMappedModel> {

	private Logger LOGGER = Pivot2SqlPlugin.getLogger(Pivot2MappedModelImpl.class);
	 ...
	 * @throws Exception
	 */
	public Pivot2MappedModelImpl(String modelInName, String outname) throws ModelAccessException {
	...
	private String query_pkNameForType(Type type) throws InvalidModelException {
		String pkname = this.ocl2DeclSettings.getPrimaryKeyPrefix() + query_nameOfGenroot(type);
		return pkname;
	}
	...
	private void map_Type2MappedClass(Type type) throws InvalidModelException {
		...
		for (Property attribute : attributes) {
			if (attribute instanceof AssociationProperty) continue;
			Guide ag = new Guide(false);
			ag.add(attribute.getName(), ovname, pkname);
			
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Creating Attribute Guide for Attribute: " + attribute.getName() + " in MappedClass " + mc.getName() + "\n" + ag);
			}
			...
			for(Type subType : subtypes) {
				...
				String ovname_sub = this.ocl2DeclSettings.getObjectViewPrefix() + typename_sub;
				...
				if (LOGGER.isDebugEnabled()) {
					LOGGER.debug("Creating Attribute Guide for Attribute: " + attribute.getName() + " in MappedClass " + sub.getName() + "\n" + ag_sub);
				}
	...
	private MappedClassImpl accessMappedClass(Type type) throws InvalidModelException {
		String typename = type.getName();
		String pkname = query_pkNameForType(type);
		String ovname = this.ocl2DeclSettings.getObjectViewPrefix() + typename;
		...
		if (this.ocl2DeclSettings.getMappedModel().isClass(typename)) { 
			mc = (MappedClassImpl) this.ocl2DeclSettings.getMappedModel().getClass(typename);
		}
		else {
			if (LOGGER.isDebugEnabled()) {
			...
			((MappedModelImpl)this.ocl2DeclSettings.getMappedModel()).addMappedClass(typename, mc);
	...
	private void map_association2guide(AssociationProperty association) throws InvalidModelException {
		...
		Type cA = association.getType();
		String nameA = association.getName();
		
		Property pB = association.getInverseAssociationProperties().get(0);
		Type cB = pB.getType();
		...
		if(!pivotModelAnalyser.isMultiple(association) && !pivotModelAnalyser.isMultiple(pB)) {
	    	map_1to1_Association2Guide(typeA, nameA, typeB, nameB);
	    }
		else if(pivotModelAnalyser.isMultiple(association) && !pivotModelAnalyser.isMultiple(pB)) {
	...
	private void map_1toN_Association2Guide(Type typeB, String nameB, Type typeA, String nameA) throws InvalidModelException {
		
		String pknameA = query_pkNameForType(typeA);
		String ovnameA = this.ocl2DeclSettings.getObjectViewPrefix() + typeA.getName();
		...
		for (Type subType : subtypesA) {
			...
			String ovnameA_sub = this.ocl2DeclSettings.getObjectViewPrefix() + subType.getName();
	...
	private void map_MtoN_Association2Guide(Type typeA, String nameA, Type typeB, String nameB, String assTableName) throws InvalidModelException {
		
		String pknameA = this.ocl2DeclSettings.getPrimaryKeyPrefix() + typeA.getName();
		String pknameB = this.ocl2DeclSettings.getPrimaryKeyPrefix() + typeB.getName();
		String ovnameA = this.ocl2DeclSettings.getObjectViewPrefix() + typeA.getName();
		String ovnameB = this.ocl2DeclSettings.getObjectViewPrefix() + typeB.getName();
		String fknameA = this.ocl2DeclSettings.getForeignKeyPrefix() + nameA;
		String fknameB = this.ocl2DeclSettings.getForeignKeyPrefix() + nameB;
		Set<Type> subtypesA = query_subtypesForType(typeA);
		Set<Type> subtypesB = query_subtypesForType(typeB);
		
		MappedClassImpl mcA = (MappedClassImpl) ((MappedModelImpl)this.ocl2DeclSettings.getMappedModel()).getClass(typeA.getName());
		MappedClassImpl mcB = (MappedClassImpl) ((MappedModelImpl)this.ocl2DeclSettings.getMappedModel()).getClass(typeB.getName());
		...
		for (Type subType : subtypesB) {
			...
			Guide sb2a = new Guide(true);
			String ovnameB_sub = this.ocl2DeclSettings.getObjectViewPrefix() + subType.getName();
	...
	private void map_1to1_Association2Guide(Type typeA, String nameA, Type typeB, String nameB) throws InvalidModelException {
