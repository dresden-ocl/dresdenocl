...
public class Cwm2DdlImpl extends M2XTransformation<Schema, String>{
	...
	private void map_fk2ddlFkConstraint(ForeignKey fk) {
		ITemplate createFKCon = this.ocl2DeclSettings.getTemplateGroup().getTemplate("createForeignKeyConstraint");
	...
	private Set<ForeignKey> query_FKForTable(Table table) {
		return cwmModelAnalyser.getInstancesOfType(table.getOwnedElement(), ForeignKey.class);
	}

	private Set<Column> query_PKColumnsForTable(Table table) {
		Set<Column> pk_columns = new HashSet<Column>();
		Set<PrimaryKey> pks = cwmModelAnalyser.getInstancesOfType(table.getOwnedElement(), PrimaryKey.class);
		for (PrimaryKey key : pks) {
			pk_columns.addAll(cwmModelAnalyser.getInstancesOfType(key.getFeature(), Column.class));
		}
		return pk_columns;
	}
	...
	private void map_view2ddlView(View view) {
		...
		ITemplate createView = this.ocl2DeclSettings.getTemplateGroup().getTemplate("createView");
	...
	private void map_table2ddlTable(Table table) {
		...
		for (Column column : pk_columns) {
			ITemplate createColumn = this.ocl2DeclSettings.getTemplateGroup().getTemplate("createColumn");
		...
		for (Column column : columns) {
			ITemplate createColumn = this.ocl2DeclSettings.getTemplateGroup().getTemplate("createColumn");
			createColumn.setAttribute("name", column.getName());
			createColumn.setAttribute("type", getTypeString(column.getType().getName()));
			concreteColumns.add(createColumn.toString());
		}
	
		String columnsString = TemplateHelper.getValuesCommaSeparated(concreteColumns);

		
		ITemplate createTable = this.ocl2DeclSettings.getTemplateGroup().getTemplate("createTable");
	...
	private Set<Column> query_columnsForFK(ForeignKey fk) {
		Set<Column> columns = cwmModelAnalyser.getInstancesOfType(fk.getFeature(), Column.class);
		return columns;
	}

	private Set<Column> query_ColumnsForTable(Table table) {
		Set<Column> columns = cwmModelAnalyser.getInstancesOfType(table.getOwnedElement(), Column.class);
		return columns;
	}
	
	private String getTypeString(String type) {
		ITemplate createType = this.ocl2DeclSettings.getTemplateGroup().getTemplate("createType");
