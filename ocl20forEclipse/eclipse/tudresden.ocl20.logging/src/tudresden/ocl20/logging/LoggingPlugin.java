 ...
 * Having a separate log manager (and an associated log4j {@link Hierarchy})
 * for each client plugin is necessary for a clean integration with the Eclipse
 ...
 * Eclipse plug-ins"</em> by Manoel
 * Marques available at the IBM developerWorks site. Large portions of the code
 * and its behaviour have been changed, some aspects removed and some features
 * added.
 * </p>
 * 
 * <p>
 ...
 * </pre>
 * 
 * and the <code>getLogger()</code> method in <code>MyPlugin</code> will
 * deal with getting the <code>ILogManager</code>. See the Javadoc for
...
public class LoggingPlugin extends Plugin {
   ...
   * 
   * @param aPlugin the plugin, may be <code>null</code>
   * 
   ...
   * when a client plugin does not want to use the {@link #getLogManager()} API
   ...
   * or accessible via the plugin's classloader. In this file, they may also use
   ...
   * </p>
   * 
   * @param aPlugin the plugin that provides additional log4j properties
   */
  public static void configureDefaultLogging(final Plugin aPlugin) {
    ...
    url = aPlugin.getBundle().getEntry("/" + DEFAULT_LOG4J_CONFIGURATION_FILE); //$NON-NLS-1$
