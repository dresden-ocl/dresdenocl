...
trait Attribution extends AttributionBase {
    ...
    class CachedAttribute[T <: Attributable,U] (f : T ==> U) extends (T ==> U) {
        ...
        def apply (t : T) : U = {
            ...
            memo.get (t) match {
                case Some (None)     => try { throw new IllegalStateException ("Cycle detected in attribute evaluation") } catch {case e : IllegalStateException => e.printStackTrace; throw e}
                case Some (Some (u)) => u
                case None =>
                    memo (t) = None
                    val u = f (t)
    ...
    class UncachedAttribute[T <: Attributable,U] (f : T ==> U) extends (T ==> U) {
        ...
        def apply (t : T) : U = {
            if (visited contains t) {
                try { throw new IllegalStateException ("Cycle detected in attribute evaluation") } catch {case e : IllegalStateException => e.printStackTrace; throw e}
            } else {
                visited (t) = ()
                val u = f (t)
                visited -= t
