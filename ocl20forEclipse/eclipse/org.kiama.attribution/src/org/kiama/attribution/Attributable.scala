...
trait Attributable extends Product with Positional {
    ...
    protected def setChildConnections = {
    	if (DEBUG)
    	  println("setChildConnections for " + this + "; productArity = " + productArity)
        for (i <- 0 until productArity) {
            if (DEBUG)
              println("product element " + i + ": " + productElement(i))
        ...
        if (DEBUG)
          println("finished set childConnections for " + this)
