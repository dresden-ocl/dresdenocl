...
WHERE NOT (((SELECT
 ...
 WHERE FK_ownedCars IN (SELECT PK_Car FROM OV_Car WHERE PK_Car = SELF.PK_Car)))))) ) = (1))));
...
WHERE NOT (((SELECT
 ...
 WHERE FK_owner IN (SELECT PK_Person FROM OV_Person
 WHERE PK_Person IN (SELECT FK_owner FROM ASS_ownedCars_owner
 WHERE FK_ownedCars = SELF.PK_Car))) ) = (1))));
...
WHERE NOT (NOT EXISTS (
  SELECT PK_Car FROM OV_Car
   ...
   WHERE FK_owner IN (SELECT PK_Person FROM OV_Person
   WHERE PK_Person IN (SELECT FK_owner FROM ASS_ownedCars_owner
   WHERE FK_ownedCars = SELF.PK_Car)))
  AND PK_Car IN (
    SELECT PK_Car FROM OV_Car AS ALIAS4
    WHERE NOT (PK_Car IN
 ...
 WHERE FK_owner IN (SELECT PK_Person FROM OV_Person
 WHERE PK_Person IN (SELECT FK_owner FROM ASS_ownedCars_owner
 WHERE FK_ownedCars = ALIAS4.PK_Car))))
  )
)));
