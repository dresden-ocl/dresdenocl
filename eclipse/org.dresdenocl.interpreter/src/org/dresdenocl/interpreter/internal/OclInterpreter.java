/*
Copyright (C) 2010,2013 by Claas Wilke (claaswilke@gmx.net)
...
public class OclInterpreter extends ExpressionsSwitch<OclAny> implements
	 ...
	 */
	protected Stack<IInterpretationEnvironment> myEnvironmentStack = new Stack<IInterpretationEnvironment>();
	...
	protected OclAny interpretConstraint(Constraint constraint, OclAny context) {
		...
		myEnvironment.setVariableValue(IOclInterpreter.SELF_VARIABLE_NAME,
				context);
	...
	public void preparePostConditions(
		...
		for (Constraint aConstraint : postConditions) {
			...
			if (aConstraint.getKind() == ConstraintKind.POSTCONDITION
				...
				/* Add self variable to environment. */
				myEnvironment.setVariableValue(SELF_VARIABLE_NAME,
						oclModelObject);
	...
	public OclAny caseIntegerLiteralExp(IntegerLiteralExp integerLiteralExp) {
		...
		/* Cache is not efficient here. */
		OclAny result = myStandardLibraryFactory.createOclInteger(Long
	...
	public OclAny caseIterateExp(IterateExp iterateExp) {
		...
		myEnvironment.deleteVariableValue(iterateExp.getResult()
				.getQualifiedName());
	...
	protected OclAny evaluateExists(OclExpression body,
		...
		/* Else compute the result. */
			...
			while (it.hasNext().isTrue()) {
				...
				/* Remove the variable from the environment again. */
				myEnvironment.deleteVariableValue(iterators.get(0)
						.getQualifiedName());
		...
		/* Else compute the result. */
			...
			while (sourceIt.hasNext().isTrue()) {
				...
				/* Else probably count the elements. */
					...
					/*
					 * If already found two elements, break and return true (one
					 * element is not sufficient, as the one iterator checks if
	...
	protected OclAny evaluateReject(OclExpression body,
		...
		/* Else compute the result. */
			...
			while (it.hasNext().isTrue()) {
				...
				result = evaluateRejectElement(body, source, iterator, it,
	...
	/**
	 * Helper method evaluating the body {@link Expression} of an reject
		...
		for (TupleLiteralPart literalPart : tupleLiteralExp.getPart()) {
			partNames.add(BasisJavaModelInstanceFactory
					.createModelInstanceString(literalPart.getProperty()
							.getName()));
			partValues.add(evaluateTupleLiteralPart(literalPart,
					tupleLiteralExp).getModelInstanceElement());
		}
		// end for.
	...
	/**
	 * Helper method evaluating a given {@link TupleLiteralPart}. Required to
	 * allow a debugger to hook in before and after the interpretation of
	 * individual {@link TupleLiteralPart}s.
	 * 
	 * @param tupleLiteralPart
	 * @param tupleLiteralExp
	 *            The {@link TupleLiteralExp} the given {@link TupleLiteralPart}
	 *            belongs to.
	 * @return The {@link OclAny} result of the given {@link TupleLiteralPart}
	 */
	protected OclAny evaluateTupleLiteralPart(
			TupleLiteralPart tupleLiteralPart, TupleLiteralExp tupleLiteralExp) {
		return caseTupleLiteralPart(tupleLiteralPart);
	}
