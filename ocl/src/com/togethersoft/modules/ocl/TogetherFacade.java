...
public class TogetherFacade implements tudresden.ocl.check.types.ModelFacade {
    ...
    public TogetherFacade(RwiElement context) {
        if (context != null && (context.getProperty(RwiProperty.SHAPE_TYPE) == RwiShapeType.CLASS ||
            context.getProperty(RwiProperty.SHAPE_TYPE) == RwiShapeType.OPERATION)) 	{
    ...
    public Any getClassifier(String name) throws OclTypeException{
        Any result = null;
        SearchForClassifierVisitor classifierVisitor = new SearchForClassifierVisitor(name);
        // Make sure to start search from a context, that represents a classifier.
        RwiElement classifierContext;
        if (context != null && context.getProperty(RwiProperty.SHAPE_TYPE).equals(RwiShapeType.OPERATION)) {
        ...
        if (classifierContext != null && (
            ...
            classifierContext.getProperty(RwiProperty.FULL_NAME).equals(name))) {
            return new TogetherAny(classifierContext);
        }

        // then check rest of package...
        if (classifierContext != null) {
	        RwiPackage currentPackage = ((RwiContainer)classifierContext).getContainingPackage();
    	    // visit the package non-recursively
        	RwiNode resultNode = (RwiNode)classifierVisitor.visitPackage(currentPackage, false);
	        if (resultNode != null) {
        ...
        if (context != null && context.getProperty(RwiProperty.SHAPE_TYPE).equals(RwiShapeType.OPERATION)) {
	        RwiElement targetRwiElement = findClassifierInOperationContext(name);
...
*/
        ...
        while (rpe.hasMoreElements() && foundRwiElement == null) {
			foundRwiElement = (RwiNode)classifierVisitor.visitPackage(rpe.nextRwiPackage());
        }
        if (foundRwiElement != null) {
            // Check, whether that node is imported or not
            if (foundRwiElement.getProperty(RwiProperty.MODEL_PART).equals("model")) {
        ...
        if (result != null) {
            ...
            throw new OclTypeException("Could not find Classifier "+name+" in the current model");
        }
    }
    ...
    public RwiElement findClassifierInOperationContext(String name) {
        if (context.getProperty(RwiProperty.SHAPE_TYPE).equals(RwiShapeType.OPERATION)) {

			// check return type first
            if (context.getProperty(RwiProperty.RETURN_TYPE).equals(name)) {
				RwiModel myModel = RwiModelAccess.getModel();
                RwiElement foundElement = myModel.findElement(context.getProperty(RwiProperty.RETURN_TYPE_REFERENCED_ELEMENT));
                return foundElement;
            }

            // then check parameters
            RwiPropertyEnumeration rpe = context.properties(RwiProperty.PARAMETER);
            while (rpe.hasMoreElements()) {
                RwiProperty someParameter = rpe.nextRwiProperty();
                RwiPropertyMap subpropertiesContainer = someParameter.getSubproperties();
                if (subpropertiesContainer.getProperty(RwiProperty.TYPE).equals(name)) {
					RwiModel myModel = RwiModelAccess.getModel();
                	RwiElement foundElement = myModel.findElement(subpropertiesContainer.getProperty(RwiProperty.TYPE_REFERENCED_ELEMENT));
    ...
    class SearchForClassifierVisitor extends RwiVisitorAdapter {
        ...
        public Object visitPackage(RwiPackage rwiPackage, boolean recursing) {
            ...
            while (rne.hasMoreElements()) {
                ...
                if (someRwiNode.getProperty(RwiProperty.NAME).equals(classifierName) ||
                    someRwiNode.getProperty(RwiProperty.FULL_NAME).equals(classifierName)) {
					return someRwiNode;
    	        }
		    }
