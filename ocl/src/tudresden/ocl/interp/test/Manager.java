...
public class Manager extends Person {
   ...
   */
  public Set managedCompanies;

  Manager(String firstName, String lastName, Calendar birthDate, boolean sex) {
    super(firstName, lastName, birthDate, sex);
    this.managedCompanies = new HashSet();
  }

  public void manage(Company c) {
    if (managedCompanies.contains(c)) {
      throw new RuntimeException();
    }

    if (!employer.contains(c)) {
      throw new RuntimeException();
    }

    // unmanage the old manager
    if (c.manager != null) {
      c.manager.managedCompanies.remove(c);
    }

    managedCompanies.add(c);
    c.manager = this;
