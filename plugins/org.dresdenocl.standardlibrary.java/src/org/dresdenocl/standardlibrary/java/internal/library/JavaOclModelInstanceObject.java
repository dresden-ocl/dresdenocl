/**
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * Copyright (C) 2007
 * Ronny Brandt (Ronny_Brandt@web.de). * All rights reserved. * * This work was done as a project at
 * the Chair for Software Technology, * Dresden University Of Technology, Germany
 * (http://st.inf.tu-dresden.de). * It is understood that any modification not identified as such is
 * not * covered by the preceding statement. * * This work is free software; you can redistribute it
 * and/or modify it * under the terms of the GNU Library General Public License as published * by
 * the Free Software Foundation; either version 2 of the License, or * (at your option) any later
 * version. * * This work is distributed in the hope that it will be useful, but WITHOUT * ANY
 * WARRANTY; without even the implied warranty of MERCHANTABILITY or * FITNESS FOR A PARTICULAR
 * PURPOSE. See the GNU Library General Public * License for more details. * * You should have
 * received a copy of the GNU Library General Public License * along with this library; if not, you
 * can view it online at * http://www.fsf.org/licensing/licenses/gpl.html. * * To submit a bug
 * report, send a comment, or get the latest news on this * project, please visit the website:
 * http://dresden-ocl.sourceforge.net. * For more information on OCL and related projects visit the
 * OCL Portal: * http://st.inf.tu-dresden.de/ocl * * * * * * * * * * * * * * * * * * * * * * * * * *
 * * * * * * * * * * * * * *
 */
 ...
 */
public class JavaOclModelInstanceObject extends JavaOclAny implements OclModelInstanceObject,
    IAddableElement {
  ...
  public OclAny add(OclAny that) {
    ...
    /*
     * convention: any ModelInstanceElement that has an operation called "+", "add" or "plus" is
        ...
        if (result == null)
          ...
          result =
              JavaStandardLibraryFactory.INSTANCE.createOclInvalid(metaType,
                  new NoSuchMethodException("Cannot find operation +, add or plus on " + this));
      }
    }
  ...
  public OclSet<OclModelInstanceObject> asSet() {
    ...
    if (result == null) {
      ...
      result =
          JavaStandardLibraryFactory.INSTANCE.createOclSet(resultSet, EssentialOclPlugin
              .getOclLibraryProvider().getOclLibrary().getSetType(metaType));
    }
   ...
   * (non-Javadoc)
   * 
   * @see org.dresdenocl.essentialocl.standardlibrary.OclModelInstanceObject
  ...
  public OclAny getProperty(Property property) {
    ...
    if (this.oclIsInvalid().isTrue()) {
      result =
          JavaStandardLibraryFactory.INSTANCE.createOclInvalid(property.getType(),
      ...
      result =
          JavaStandardLibraryFactory.INSTANCE.createOclInvalid(property.getType(),
              new NullPointerException(this.getUndefinedReason()));
    }
    ...
    /* Else try to get the property. */
      ...
      try {
        ...
        if (imiResult.isUndefined()) {
          result =
              JavaStandardLibraryFactory.INSTANCE.createOclUndefined(property.getType(),
  ...
  protected OclAny checkInvalid(Operation operation, OclAny... args) {
    ...
    /*
     * see standard, p. 138: all operations on OclInvalid are illegal, except oclIsInvalid();
     * oclIsUndefined() is not considered to catch invalid -> this is not conform to the standard!
     */
    if (this.oclIsInvalid().isTrue()
        && !(operation.getName().equals("oclIsInvalid") && args.length == 0)
        && !(operation.getName().equals("isEqualTo") && args.length == 1)) {
      result =
          JavaStandardLibraryFactory.INSTANCE.createOclInvalid(operation.getType(),
              this.getInvalidReason());
    }

    /*
     * see standard, p. 138: all operations on OclVoid are illegal, except oclIsInvalid() and
     * oclIsUndefined()
     */
    else if (this.oclIsUndefined().isTrue()
        && !(operation.getName().equals("oclIsInvalid") && args.length == 0)
  ...
  private OclAny findMethod(String methodName, OclAny that) {
    ...
    try {
      ...
      if (operation != null)
        ...
        result =
            JavaStandardLibraryFactory.INSTANCE.createOclInvalid(metaType,
                new NoSuchMethodException("Cannot find " + methodName + " on " + this));
